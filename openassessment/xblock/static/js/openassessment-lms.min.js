...
OpenAssessment.ResponseView.prototype = {
    ...
    installHandlers: function() {
        ...
        var handlePrepareUpload = function(eventData) {
            view.prepareUpload(eventData.target.files)
        };
    ...
    prepareUpload: function(files) {
        ...
        if (files[0].size > this.MAX_FILE_SIZE) {
        	...
        	console.log("subiendo imagen ok")
            this.baseView.toggleActionError("upload", gettext("File must be an image."))
        } else {
            this.baseView.toggleActionError("upload", null);
            this.files = files
        }
        if  (this.imageType ==  "application/pdf"){
        	console.log("subiendo pdf ok")s
        	this.baseView.toggleActionError("upload", null);
            this.files = files
        }
    ...
    fileUpload: function() {
        ...
        this.server.getUploadUrl(view.imageType).done(function(url) {
            ...
            view.fileUploader.upload(url, image).done(function() {
                view.imageUrl();
...
OpenAssessment.StaffInfoView.prototype = {
    ...
    installHandlers: function() {
        ...
        sel.find("#submit_student_username").click(function(eventObject) {
            eventObject.preventDefault();
            view.loadStudentInfo()
        });
